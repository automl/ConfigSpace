...
def kosaraju_strongly_connected_components(G,source=None):
    ...
    post=list(HPOlibConfigSpace.nx.dfs_postorder_nodes(G,source=source))
    G=G.reverse(copy=False)
    seen={}
    while post:
        r=post.pop()
        if r in seen:
            continue
        c= HPOlibConfigSpace.nx.dfs_preorder_nodes(G,r)
...
def is_strongly_connected(G):
    ...
    if len(G)==0:
        raise HPOlibConfigSpace.nx.NetworkXPointlessConcept(
...
def condensation(G, scc=None):
    ...
    if not G.is_directed():
        raise HPOlibConfigSpace.nx.NetworkXError("""Not allowed for undirected graph G.
              See is_connected() for connectivity test.""")
    if scc is None:
        scc = HPOlibConfigSpace.nx.strongly_connected_components(G)
    mapping = {}
    C = HPOlibConfigSpace.nx.DiGraph()
