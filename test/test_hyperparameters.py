...
class TestHyperparameters(unittest.TestCase):
    ...
    def test_uniformfloat(self):
        ...
        self.assertEqual("param, Type: UniformFloat, Range: [0.0, 10.0], Q: 0.1",
                         str(f2))

        f3 = UniformFloatHyperparameter("param", 0, 10, log=True)
        f3_ = UniformFloatHyperparameter("param", 0, 10, log=True)
        self.assertEqual(f3, f3_)
        self.assertEqual(
            "param, Type: UniformFloat, Range: [0.0, 10.0], "
            "on log-scale", str(f3))

        f4 = UniformFloatHyperparameter("param", 0, 10, q=0.1, log=True)
        f4_ = UniformFloatHyperparameter("param", 0, 10, q=0.1, log=True)
        self.assertEqual(f4, f4_)
        self.assertEqual(
            "param, Type: UniformFloat, Range: [0.0, 10.0], "
            "on log-scale, Q: 0.1", str(f4))

        self.assertNotEqual(f1, f2)
        self.assertNotEqual(f1, "UniformFloat")

    def test_uniformfloat_to_integer(self):
        f1 = UniformFloatHyperparameter("param", 0, 10, q=0.1, log=True)
        with warnings.catch_warnings():
            f2 = f1.to_integer()
            warnings.simplefilter("ignore")
            self.assertEqual("param, Type: UniformInteger, Range: [0, 10], "
                             "on log-scale", str(f2))
    ...
    def test_normalfloat(self):
        ...
        self.assertEqual(f1, f1_)
        self.assertEqual(
            "param, Type: NormalFloat, Mu: 0.5 Sigma 10.5", str(f1))

        f2 = NormalFloatHyperparameter("param", 0, 10, q=0.1)
        f2_ = NormalFloatHyperparameter("param", 0, 10, q=0.1)
        self.assertEqual(f2, f2_)
        self.assertEqual(
            "param, Type: NormalFloat, Mu: 0.0 Sigma 10.0, Q: 0.1",
            str(f2))
        ...
        self.assertEqual(f3, f3_)
        self.assertEqual(
            "param, Type: NormalFloat, Mu: 0.0 Sigma 10.0, "
            "on log-scale", str(f3))

        f4 = NormalFloatHyperparameter("param", 0, 10, q=0.1, log=True)
        f4_ = NormalFloatHyperparameter("param", 0, 10, q=0.1, log=True)
        self.assertEqual(f4, f4_)
        self.assertEqual(
            "param, Type: NormalFloat, Mu: 0.0 Sigma 10.0, "
            "on log-scale, Q: 0.1", str(f4))
    ...
    def test_normalfloat_to_uniformfloat(self):
        ...
        f2 = NormalFloatHyperparameter("param", 0, 10, q=0.1, log=True)
        f2_expected = UniformFloatHyperparameter("param", -30, 30, q=0.1, log=True)
    ...
    def test_uniforminteger(self):
        f1 = UniformIntegerHyperparameter("param", 0.5, 5.5)
        f1_ = UniformIntegerHyperparameter("param", 0.5, 5.5)
        self.assertEqual(f1, f1_)
        ...
        self.assertEqual(f2, f2_)
        self.assertEqual(
            "param, Type: UniformInteger, Range: [0, 10]",
            str(f2))
        ...
        self.assertEqual(
            "param, Type: UniformInteger, Range: [0, 10], Q: 2",
            str(f2_large_q))

        f3 = UniformIntegerHyperparameter("param", 0, 10, log=True)
        f3_ = UniformIntegerHyperparameter("param", 0, 10, log=True)
        self.assertEqual(f3, f3_)
        self.assertEqual(
            "param, Type: UniformInteger, Range: [0, 10], "
            "on log-scale", str(f3))

        f4 = UniformIntegerHyperparameter("param", 0, 10, q=0.1, log=True)
        f4_ = UniformIntegerHyperparameter("param", 0, 10, q=0.1, log=True)
        self.assertEqual(f4, f4_)
        self.assertEqual(
            "param, Type: UniformInteger, Range: [0, 10], "
            "on log-scale", str(f4))
    ...
    def test_normalint(self):
        ...
        self.assertEqual(f2_large_q, f2_large_q_)
        self.assertEqual(
            "param, Type: NormalInteger, Mu: 0 Sigma 10, Q: 2",
            str(f2_large_q))
        ...
        f4 = NormalIntegerHyperparameter("param", 0, 10, q=0.1, log=True)
        f4_ = NormalIntegerHyperparameter("param", 0, 10, q=0.1, log=True)
        self.assertEqual(f4, f4_)
        self.assertEqual(
            "param, Type: NormalInteger, Mu: 0 Sigma 10, "
            "on log-scale", str(f4))
    ...
    def test_normalint_to_uniform(self):
        ...
        f1_expected = UniformIntegerHyperparameter("param", -30, 30, log=True)
        f1_actual = f1.to_uniform()
        self.assertEqual(f1_expected, f1_actual)
    ...
    def test_categorical(self):
        ...
        self.assertEqual(f1, f1_)
        self.assertEqual("param, Type: Categorical, Choices: {0, 1}",
                         str(f1))
        ...
        self.assertEqual(f2, f2_)
        self.assertEqual(
            "param, Type: Categorical, Choices: {%s}" %
    ...
    def test_categorical_strings(self):
        ...
        self.assertEqual(f1, f1_)
        self.assertEqual("param, Type: Categorical, Choices: {a, b}", str(f1))
